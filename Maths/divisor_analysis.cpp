...
void solve()
{
    int n;
    cin >> n;
    int num1 = 1, num2 = 1, sum=1, prod=1;
    fr(0, n){
        int x, k;
        cin >> x >> k;
        num1 = (num1 * (k+1)) % MOD;
        sum = sum*(fastpow(x, k+1)-1) % MOD * fastpow(x-1, MOD-2) % MOD;
        prod = fastpow(prod, k+1) * fastpow(fastpow(x, k*(k+1)/2), num2) % MOD;
        num2 = num2*(k+1) % (MOD-1);
    }
    cout << num1 << " " << sum << " " << prod << endl;
